...
/* --- Sysfs: Camera Reset Command --- */

/*
 * ur_do_reset() - Toggle reset GPIO and restart the stream if it was active.
 *
 * Sequence:
 *   1. Stop stream (if streaming)
 *   2. Assert reset GPIO low  (active-low reset)
 *   3. Hold for 20 ms
 *   4. De-assert reset GPIO high
 *   5. Wait 50 ms for camera to boot
 *   6. Restart stream (if it was streaming before)
 *
 * Called from sysfs write handler (user-space) or internal error recovery.
 */
static int ur_do_reset(struct ur_sensor *sensor)
{
    struct i2c_client *client = sensor->client;
    bool was_streaming;
    int ret = 0;
    ...
    /* 1. Stop stream without grabbing mutex (already held) */
    if (was_streaming) {
        u8 *buf;
        unsigned short crc;
        ...
        ur_write_buffer(client, UR_REG_BUFFER_RW, buf, sizeof(stop_regs_template));
        kfree(buf);
        sensor->streaming = false;
    }

    /* 2-5. GPIO reset pulse */
    if (sensor->reset_gpio) {
        dev_info(&client->dev, "Camera reset: asserting reset GPIO\n");
        gpiod_set_value_cansleep(sensor->reset_gpio, 0); /* assert reset */
        msleep(20);
        gpiod_set_value_cansleep(sensor->reset_gpio, 1); /* de-assert */
        msleep(50);                                       /* boot time   */
        dev_info(&client->dev, "Camera reset: GPIO released, camera booting\n");
    } else {
        dev_warn(&client->dev, "Camera reset: no reset GPIO available\n");
    }

    /* 6. Restart stream if it was active */
    if (was_streaming) {
        u8 *buf;
        unsigned short crc;
        ...
        dev_info(&client->dev, "Camera reset: restarting stream\n");
        ret = ur_write_buffer(client, UR_REG_BUFFER_RW, buf,
                              sizeof(start_regs_template));
        kfree(buf);

        if (!ret) {
            msleep(50);
            sensor->streaming = true;
            dev_info(&client->dev, "Camera reset: stream restored\n");
        } else {
            dev_err(&client->dev, "Camera reset: failed to restart stream: %d\n", ret);
        }
    }
...
/*
 * Sysfs attribute: /sys/bus/i2c/devices/<bus>-003c/reset_camera
 *
 * Usage:
 *   echo 1 > /sys/bus/i2c/devices/9-003c/reset_camera
 */
static ssize_t reset_camera_store(struct device *dev,
                                  ...
                                  const char *buf, size_t count)
{
    ...
    /*
     * dev_get_drvdata() points at s_data->subdev (set in ur_init_camera_common_data).
     * Convert it back to ur_sensor via our helper.
     */
    if (!sd)
        return -ENODEV;

    sensor = to_ur_sensor(sd);
    ...
    ret = ur_do_reset(sensor);
    return ret ? ret : count;
}

static DEVICE_ATTR_WO(reset_camera);

static struct attribute *ur_sysfs_attrs[] = {
    &dev_attr_reset_camera.attr,
    NULL,
};

static const struct attribute_group ur_sysfs_group = {
    .attrs = ur_sysfs_attrs,
};
...
static int ur_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* 7. Expose sysfs reset_camera attribute */
    ret = sysfs_create_group(&client->dev.kobj, &ur_sysfs_group);
    if (ret)
        dev_warn(&client->dev, "Failed to create sysfs group: %d\n", ret);
    else
        dev_info(&client->dev,
                 "sysfs reset: echo 1 > /sys/bus/i2c/devices/%s/reset_camera\n",
                 dev_name(&client->dev));
...
static int ur_remove(struct i2c_client *client)
{
    ...
    sysfs_remove_group(&client->dev.kobj, &ur_sysfs_group);
